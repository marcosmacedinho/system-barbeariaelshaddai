...
const goBack = () => {
    router.replace('/'); // Volta para a tela anterior
};
...
onMounted(async () => {
    await fetchUserRole();
    subscribeToComments(); // Inscrever-se em comentários
});
</script>

<template>
    <div class="comment-section">
        <h3 class="section-title d-flex align-items-center gap-1" @click="goBack">
        ...
        <div class="comments-container">
            <div v-for="comment in comments" :key="comment.id" class="comment-bubble"
                :class="{ 'my-comment': comment.authorId === currentUser.id, 'other-comment': comment.authorId !== currentUser.id }">
                <div class="comment-header">
                    ...
                    <div class="comment-info">
                        ...
                        <span class="comment-date">{{ new Date(comment.createdAt).toLocaleTimeString([], {
                ...
                <div v-if="isAuthorOrAdmin(comment.id)" class="actions">
                    ...
                    <div v-if="activeMenuId === comment.id" class="action-menu">
                        ...
                        <button @click="editComment(comment.id, prompt('Edite seu comentário:', comment.text))"
                            class="action-button">Editar</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="input-container">
            <input v-model="newComment" @keyup.enter="submitComment" placeholder="Digite uma mensagem..."
                class="comment-input" :disabled="!currentUser" />
