            ...
            <div class="card appointment-card mb-3" v-for="appointment in filteredAppointments" :key="appointment.id">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div class="d-flex flex-column">
                        ...
                        <span :class="getStatusClass(appointment.status)" class="status-badge">{{ appointment.status }}</span>
                    </div>
    ...
    try {
        ...
        onSnapshot(q, (querySnapshot) => {
            appointments.value = querySnapshot.docs.map(doc => {
                const data = doc.data();
                console.log('Data do agendamento:', data.dayDate); // Adicione este log
                return {
                    id: doc.id,
                    name: data.name,
                    phone: data.phone,
...
const formatDate = (dateString) => {
    ...
    const dateValue = new Date(dateString + 'T00:00:00-03:00'); // O "-03:00" representa o UTC-3 para Brasília
...
const clearAppointments = async (filter) => {
    try {
        ...
        let filteredList = [];
        if (filter === 'hour') {
            // Limpa agendamentos na última hora
            ...
            // Limpa agendamentos do último mês
            const oneMonthAgo = new Date(now.setMonth(now.getMonth() - 1)); // Último mês
        ...
        // Commit the batch operation to delete
        await batch.commit();

        // Atualiza localmente a lista de agendamentos removendo os que foram excluídos
...
const cancelAppointment = async (id) => {
    try {
        const docRef = doc(db, 'bookings', id);
        // Atualizando o status para 'despachado' ao invés de deletar
        await updateDoc(docRef, { status: 'despachado' });
        // Atualizando localmente o status do agendamento na lista
        appointments.value = appointments.value.map(app =>
            app.id === id ? { ...app, status: 'despachado' } : app
        );
